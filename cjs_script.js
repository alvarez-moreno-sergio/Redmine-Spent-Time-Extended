...
function updateIcon(){
	if ($("#playResumeSpentTime").attr('class') === "play"){
...
let spentTime;
if (~window.location.href.lastIndexOf('/issues/')){
    if (isIssueAssignedToCurrentUser()){
        ...
        $("#playResumeSpentTime").on('click', (e) => {
        	spentTime = JSON.parse(localStorage.getItem("spentTime"));
        	let lastWorkingIssue = localStorage.getItem("lastWorkingIssue");
        	let issueId = $("div#content h2").text().substring($("div#content h2").text().indexOf("#")+1);
        	if (lastWorkingIssue === issueId){
        		updateIcon();
        		console.log("Tried to start working on last Issue the user was working in = "+lastWorkingIssue);
        	}
        	else if (work()){
        		updateIcon();
        	}
